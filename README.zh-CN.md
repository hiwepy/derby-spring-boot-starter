...
**derby-spring-boot-starter** 是一个面向 使用 derby 的应用 的 Spring Boot Starter，用于将 **derby** 集成到 Spring Boot 应用中。它提供自动装配、属性绑定与开箱即用的 Bean，使应用以最小配置即可使用 derby 的全部能力。

| 维度 | 说明 |
|---|---|
| 类型 | Spring Boot Starter |
| 消费方 | 使用 derby 的 Spring Boot 应用 |
| 核心能力 | 自动装配、属性绑定、开箱即用的 derby Bean |
| JDK | `17` |
| 坐标 | `io.github.easy4j:derby-spring-boot-starter:3.5.x.20260527-SNAPSHOT` |
| 配置前缀 | `derby` |
...
| 能力 | 状态 | 说明 |
|---|:---:|---|
| 自动装配 | ✅ 稳定 | 自动注册 derby 相关 Bean |
| 属性绑定 | ✅ 稳定 | 绑定 `derby.*` 到 `DerbyProperties` |
| 开箱即用的 Bean | ✅ 稳定 | 通过 DerbyAutoConfiguration 自动注册 |
...
| 属性 | 类型 | 默认值 | 必填 | 说明 | 敏感 |
|---|---|---|:---:|---|:---:|
| `derby.enabled` | boolean | `true` | 否 | 是否启用 Starter | 否 |
<!-- 更多属性见下方 -->
...
| 症状 | 诊断 | 解决 |
|---|---|---|
| Bean 未创建 | 查看自动装配报告 | 确认 `derby.enabled=true` 与 classpath |
...
[返回顶部](#readme-top) · [问题反馈](https://github.com/easy-4-java/derby-spring-boot-starter/issues) · [仓库地址](https://github.com/easy-4-java/derby-spring-boot-starter)
